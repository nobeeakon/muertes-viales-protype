...
function ParseAndAddNotes() {
  ...
  const inputRef= useRef<HTMLInputElement>(null)
    ...
    const onUrlsChange = (event:  React.ChangeEvent<HTMLInputElement>) => {
        const newUrls = event.target.value.split(',').map(urlItem => urlItem.trim()).filter(Boolean)
  ...
  return (
    ...
    <form onSubmit={saveUrls}>
       <input
        ref={inputRef}
        type='text  '
        value={urls} onChange={onUrlsChange}/>
        <button type='submit'>Guardar</button>
        </form>
<ol>

{urls.split(',').length > 0 && urls.split(',').map(urlItem => <li key={urlItem}>{urlItem}</li>)}
</ol>
        </div>
    </div>
  );
